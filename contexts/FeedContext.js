...
export const FeedProvider = (props: {children: React.ReactNode}): any => {
  ...
  async function buyItem(item) {
    try {
      ...
      const royaltyAmount = 0.15 * item.price;
      const amountForSeller = 0.84 * item.price;
      const ownerAmount = 0.01 * item.price;
  ...
  const getFeedItems = async () => {
    ...
    fetch(query, {
      ...
      .then(async response => {
        ...
        const nftArray = await transformCollectionAssetsJsonResponse(
          jsonResponse.result,
        );
  ...
  const getMarketPlaceItems = async () => {
    ...
    const marketDataReturn = await contract.methods
      .fetchMarketItems()
  ...
  const transformCollectionAssetsJsonResponse = async assets => {
    ...
    for (let i = 0; i < assets.length; i++) {
      let {metadata, token_uri, token_address, owner_of, token_id, amount} =
        assets[i];
