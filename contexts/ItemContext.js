...
export const ItemContext = createContext({
  ...
  export const ItemProvider = (props: { children: React.ReactNode }): any => {
    ...
    const updateMetadata = (metaDataBody) => fetch("https://api.nftport.xyz/v0/metadata", {
        "method": "POST",
        "headers": {
            "Content-Type": "application/json",
    ...
    const handleLikePress = async (item) => {
        const {likes} = item
        console.log('includes', likes, likes.includes(walletAddress), walletAddress)
        if(!likes?.includes(walletAddress)){
            try {
                ...
                const updateMetadataURIResponse = await updateMetadataURI(response.metadata_uri, item.id)
