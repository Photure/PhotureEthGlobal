...
export const CameraProvider = (props: {children: React.ReactNode}): any => {
  ...
  const handleMetadataFailure = async (formValues, remixedItem, filePath) => {
    ...
    uploadMetadata(ipfs_url, formValues, remixedItem, filePath)
      .then(metadataResponse => {
        setMetadataURI(metadataResponse.metadata_uri);
        console.log('metadataResponse', metadataResponse);
        customMint(metadata_uri)
          .then(response => {
            onSuccess();
  ...
  const handleMint = (filePath, formValues, remixedItem) => {
    if (!wc.session.connected) {
      wc.connect({chainId: 137});
    }
    setIsLoadingModalVisible(true);
    uploadToIPFS(filePath, formValues.name)
      .then(IPFSResponse => {
        if (!IPFSResponse.ipfs_url) {
          throw new Error('IPFS Response is empty');
        }
        ...
        uploadMetadata(IPFSResponse.ipfs_url, formValues, remixedItem, filePath)
          .then(metadataResponse => {
            setMetadataURI(metadataResponse.metadata_uri);
            console.log('metadataResponse', metadataResponse);
            const {metadata_uri: metadata_url} = metadataResponse;
            customMint(metadata_url)
              .then(response => {
                setIsLoadingModalVisible(false);
                onSuccess();
                setTransactionHash(response);
  ...
  const uploadMetadata = (URLFromResponse, formValues, remixedItem, filePath) =>
    new Promise((resolve, reject) => {
      ...
      const metaDataBody = formatMetadata(
        URLFromResponse,
        formValues,
        remixedItem,
        filePath,
  ...
  const uploadToIPFS = (filePath, name) =>
    new Promise((resolve, reject) => {
