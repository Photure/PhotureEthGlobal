...
const GALLERY_ACTION_TYPES = {
  ...
  export const ProfileProvider = (props: { children: React.ReactNode }): any => {
    ...
    const connection =new Web3(new Web3.providers.HttpProvider(MUMBAI_INFURA_ENDPOINT,
      {headers: [
        {
          name: 'user', value: MUMBAI_INFURA_SECRET}
      ]}
    ))
    ...
    async function postItemForSale(item) {
      ...
      }, [ALTERNATIVE_MINTING_CONTRACT_ADDRESS, item.token_id, '1000', walletConnect.accounts[0]]);
      ...
      fetch(moralisQuery, {
        ...
        .then(async response => {
          ...
          const jsonResponse = await response.json()
            ...
            dispatch({type: GALLERY_ACTION_TYPES.SET_GALLERY_DATA, payload: nftArray})
        })
